"""Additional experiments for ModularBooth paper.

New experiments:
1. Longer training (1000 steps) for blockwise and blockwise+CCD
2. Rank sensitivity: identity rank {8, 12, 24, 32} with context=4, shared=8
3. CCD lambda ablation: lambda_ccd {0.1, 0.5, 1.0}
"""
...
logger = logging.getLogger("additional_experiments")
...
def run_experiment(config: dict):
    ...
    gpu_cleanup()
    logger.info("[%s] Starting experiment...", name)
    ...
    logger.info("[%s] LoRA params: %d", name, total_params)
    ...
    cfg = OmegaConf.create({
        ...
        "training": {
            ...
            "log_every": 50, "save_every": num_steps, "validate_every": num_steps,
            "scheduler": "cosine",
        },
    ...
    # Train
    logger.info("[%s] Training %d steps...", name, num_steps)
    t0 = time.time()
    results = trainer.train()
    train_time = time.time() - t0
    logger.info("[%s] Training: %.1fs, loss=%.4f", name, train_time, results.get("loss_total", 0))
    ...
    logger.info("[%s] avg |B|=%.6f (%d modules)", name, avg_b, n)
    ...
    logger.info("[%s] Generated %d eval + %d CAE images", name, len(gen_paths), len(cae_paths))
    ...
    try:
        ...
        logger.info("[%s] DINO: %.4f, CAE: %.6f", name, metrics["dino_score"], metrics["cae"])
        del dino, cae_scorer
        gpu_cleanup()
    except Exception as e:
        logger.warning("[%s] DINO/CAE failed: %s", name, e)
...
def get_block_configs():
    """Discover SD1.4 block structure and return uniform + blockwise configs."""
    from diffusers import StableDiffusionPipeline
    pipe = StableDiffusionPipeline.from_pretrained(
        "CompVis/stable-diffusion-v1-4", torch_dtype=torch.float16, safety_checker=None,
    )
    ...
    return uniform_config, blockwise_config


# Define all additional experiments
ALL_EXPERIMENTS = {}


def register_experiments(uniform_config, blockwise_config):
    global ALL_EXPERIMENTS

    # 1. Longer training (1000 steps)
    ALL_EXPERIMENTS["blockwise_1000"] = {
        "name": "blockwise_1000", "block_config": blockwise_config,
        "identity_rank": 16, "context_rank": 4, "shared_rank": 8,
        "num_steps": 1000, "learning_rate": 5e-4,
    }
    ALL_EXPERIMENTS["blockwise_ccd_1000"] = {
        "name": "blockwise_ccd_1000", "block_config": blockwise_config,
        "identity_rank": 16, "context_rank": 4, "shared_rank": 8,
        "num_steps": 1000, "learning_rate": 5e-4,
        "use_ccd": True, "lambda_ccd": 0.3,
    }

    # 2. Rank sensitivity ablation (identity rank varies, context=4, shared=8)
    for id_rank in [8, 12, 24, 32]:
        exp_name = f"rank_sensitivity_id{id_rank}"
        ALL_EXPERIMENTS[exp_name] = {
            "name": exp_name, "block_config": blockwise_config,
            "identity_rank": id_rank, "context_rank": 4, "shared_rank": 8,
            "num_steps": 500, "learning_rate": 5e-4,
        }

    # 3. CCD lambda ablation
    for lam in [0.1, 0.5, 1.0]:
        exp_name = f"ccd_lambda_{str(lam).replace('.', '')}"
        ALL_EXPERIMENTS[exp_name] = {
            "name": exp_name, "block_config": blockwise_config,
            "identity_rank": 16, "context_rank": 4, "shared_rank": 8,
            "num_steps": 500, "learning_rate": 5e-4,
            "use_ccd": True, "lambda_ccd": lam,
        }

    # 4. Uniform rank 8 at 1000 steps (baseline comparison for longer training)
    ALL_EXPERIMENTS["uniform_r8_1000"] = {
        "name": "uniform_r8_1000", "block_config": uniform_config,
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python run_additional_experiments.py <exp_name>")
        print("Available experiments will be listed after block config discovery.")
        uniform_config, blockwise_config = get_block_configs()
        register_experiments(uniform_config, blockwise_config)
    ...
    if exp_name not in ALL_EXPERIMENTS:
        print(f"Unknown experiment: {exp_name}. Available: {list(ALL_EXPERIMENTS.keys())}")
        sys.exit(1)

    run_experiment(ALL_EXPERIMENTS[exp_name])
