...
Two-subject composition experiments demonstrate that masked inference achieves $2.76\times$ higher identity isolation than na\"ive LoRA addition, validating the plug-and-play compositional design.
...
To validate the masked compositional inference protocol (Section~\ref{sec:masked_inference}), we train two independent subject LoRA modules---a ``sks dog'' and a ``sks cat''---each using the blockwise+CCD configuration ($r_{\text{id}}{=}16$, $r_{\text{ctx}}{=}4$, $r_{\text{sh}}{=}8$, $\lambda_2{=}0.3$, 500 steps), and compose them in two-subject scenes.

\paragraph{Setup.}
We generate 12 two-subject images (6 prompts $\times$ 2 seeds) under two composition strategies:
(1)~\textbf{Masked composition} (our method): a custom denoising loop where each subject's LoRA is applied independently, its noise prediction is spatially blended using soft masks ($\alpha{=}0.05$ leakage), and negative attention ($\gamma{=}3.0$) suppresses identity features in background regions.
The dog occupies the left half and the cat the right half of the image.
(2)~\textbf{Na\"ive LoRA addition} (baseline): both subject LoRAs are applied simultaneously to the UNet, and standard pipeline inference is used without any spatial guidance.
...
We also measure CLIP-T (prompt fidelity) and composition diversity (1 minus DINO similarity between left and right crops; higher means more distinct subjects).

\paragraph{Results.}
Table~\ref{tab:multi_subject} presents the results.
Masked composition achieves $2.76\times$ higher IIS than na\"ive addition (0.123 vs.\ 0.045), indicating substantially better identity isolation.
Cross-contamination---the similarity of each crop to the \emph{wrong} subject's references---drops markedly: from 0.269 to 0.202 for the dog and from 0.238 to 0.206 for the cat under masking.
Prompt fidelity also improves (CLIP-T 0.278 vs.\ 0.273), and composition diversity increases (0.529 vs.\ 0.477), confirming that the two halves of the image contain more distinct content.

These results validate that independently trained blockwise LoRA modules can be composed at inference via spatial masking without retraining or joint optimization.
The negative-attention mechanism suppresses identity leakage in background regions, which is the primary source of attribute mixing in the na\"ive baseline.

\begin{table}[t]
\caption{Multi-subject composition: masked inference vs.\ na\"ive LoRA addition.
Masked composition achieves $2.76\times$ higher identity isolation (IIS) and reduced cross-subject contamination.}
...
\begin{tabular}{lcccc}
\toprule
Method & IIS$\uparrow$ & CLIP-T$\uparrow$ & Comp.\ Div.$\uparrow$ & Cross-Cont.$\downarrow$ \\
\midrule
Na\"ive addition & 0.045 & 0.273 & 0.477 & 0.254 \\
Masked (ours) & \textbf{0.123} & \textbf{0.278} & \textbf{0.529} & \textbf{0.204} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\paragraph{Multi-subject composition.}
The masked inference experiment (Table~\ref{tab:multi_subject}) demonstrates that independently trained blockwise LoRA modules can be composed without retraining.
The $2.76\times$ IIS improvement over na\"ive addition stems from two mechanisms: spatial masking confines each subject's LoRA delta to its designated region, and negative attention actively suppresses identity features in the background.
Cross-contamination (average DINO similarity of each crop to the wrong subject) drops from 0.254 to 0.204---a 20\% reduction.
The composition diversity metric (0.529 vs.\ 0.477) confirms that the generated images contain spatially distinct subjects rather than a blended amalgam.
These results, while preliminary (two subjects, fixed spatial layout), validate the core compositional principle: blockwise LoRA modules trained with CCD produce subject representations that compose cleanly via spatial blending.
...
The masked compositional inference protocol enables plug-and-play multi-subject generation from independently trained modules without retraining, achieving $2.76\times$ higher identity isolation than na\"ive LoRA addition in two-subject composition experiments.
