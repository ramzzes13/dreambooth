...
ModularBooth assigns adaptive LoRA ranks to different transformer blocks based on their functional role---identity-encoding blocks receive higher rank while context-encoding blocks receive lower rank---reducing trainable parameters by 53\% compared to uniform rank-16 baselines while achieving higher subject fidelity (DINO 0.393 vs.\ 0.378).
...
The blockwise configuration ($r_{\text{id}}{=}16$, $r_{\text{ctx}}{=}4$, $r_{\text{sh}}{=}8$) uses 1.49M parameters---53\% fewer than uniform rank~16 (3.19M)---while achieving higher DINO (0.393 vs.\ 0.378) and the highest CLIP-T (0.271, tied with uniform $r{=}4$).
...
Adding CCD improves DINO (+1.0\%), CLIP-I (+1.2\%), and VQA alignment (+2.0\%) with no additional parameters.
...
DINO increases from 0.378 at $r_{\text{id}}{=}8$ to 0.393 at $r_{\text{id}}{=}16$, then plateaus (0.382 at rank~24, 0.392 at rank~32).
...
Rank~16 provides the best DINO-per-parameter ratio: 0.393 DINO at 1.49M parameters, versus 0.392 DINO at 2.00M for rank~32 (34\% more parameters for equivalent fidelity).
...
16 & 1.49M & \textbf{0.393} & \textbf{0.271} & 0.771 & 0.846 & 0.00112 \\
...
\midrule
0.0 & 0.393 & \textbf{0.271} & 0.771 & 0.846 & \textbf{0.00112} & 0.759 \\
...
Without CCD, longer training degrades prompt alignment: blockwise at 1000 steps drops CLIP-T from 0.271 to 0.258 ($-4.8\%$) and VQA from 0.846 to 0.754 ($-10.9\%$), despite increasing DINO from 0.393 to 0.418.
...
\midrule
Blockwise & 500 & 0.393 & \textbf{0.271} & 0.771 & 0.846 & 0.00112 \\
...
Blockwise LoRA with 1.49M parameters achieves DINO~0.393, within 1.3\% of the uniform rank-8 baseline (1.59M, DINO~0.398) while surpassing the $2\times$ larger uniform rank-16 (3.19M, DINO~0.378).
...
At 500 steps, the CCD loss provides modest gains: DINO +1.0\%, CLIP-I +1.2\%, VQA +2.0\% over blockwise training without CCD.
...
Third, multi-subject composition was implemented and validated architecturally (unit tests, module interfaces) but not evaluated with full generation experiments due to resource constraints.
...
The CCD loss provides dual value: at 500 steps, it improves subject fidelity metrics (DINO +1.0\%, CLIP-I +1.2\%); at 1000 steps, it acts as a regularizer that prevents prompt-alignment degradation (maintaining VQA at 0.863) while enabling significant DINO improvement (+6.0\% to 0.421).
...
The masked compositional inference protocol enables plug-and-play multi-subject generation from independently trained modules without retraining.
...
Future work will scale ModularBooth to DiT architectures (FLUX, SD3), replace heuristic block classification with learned rank allocation, extend CCD augmentations to segmentation-based background replacement, and evaluate on real-world multi-subject benchmarks.
